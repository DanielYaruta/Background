...
Пример вывода при нажатии кнопок:

```
D MyBoundService: Сервис создан (onCreate)
D MyBoundService: Сервис запущен — клиент привязался (onBind)
D MyBoundService: Клиент отвязался от сервиса (onUnbind)
D MyBoundService: Сервис завершён (onDestroy)
```
